...
export function parseViewBox(svg: SVGSVGElement): ViewBoxInfo {
...
export function parseTransform(transform: string | null | undefined): { tx: number; ty: number } {
  if (!transform) {
    return { tx: 0, ty: 0 };
  }
...
export function getAbsoluteAnchor(el: Element): { x: number; y: number } {
  const chain: Element[] = [];
...
export function parseStyleValue(style: string, prop: string): string | undefined {
