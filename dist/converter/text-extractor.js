...
import { getAbsoluteAnchor, parseStyleValue, } from './transform-utils.js';
...
function adjustTextBox(anchorX, anchorY, width, height, textAnchor, dominantBaseline) {
    let left = anchorX;
    let top = anchorY;
...
function parseHorizontalAlign(el, spanStyle) {
...
function buildTextGradientColorMark(color, textGradientColor) {
...
function toDeckCoords(x, y, viewBox) {
    return {
        left: x - viewBox.minX,
        top: y - viewBox.minY,
    };
}
function textElementToCandidate(el, defaultFontFamily, defaultFontSize, viewBox, svgRoot) {
    const content = getDirectText(el);
    if (!content) {
        return null;
    }
    const props = collectElementProps(el);
    const fontFamily = typeof props.fontFamily === 'string' ? props.fontFamily : defaultFontFamily;
    const fontSize = parseFontSize(typeof props.fontSize === 'string' || typeof props.fontSize === 'number'
        ? props.fontSize
        : undefined, defaultFontSize);
    const sizeHint = findSizeHint(el);
    const { width, height } = sizeHint.width && sizeHint.height
    ...
    const dominantBaseline = typeof props.dominantBaseline === 'string' ? props.dominantBaseline : undefined;
...
function walkTextElements(el, candidates, defaultFontFamily, defaultFontSize, viewBox, svgRoot, ctx) {
    ...
    if (tag === 'foreignobject') {
        const candidate = foreignObjectToCandidate(el, defaultFontFamily, defaultFontSize, viewBox, svgRoot);
        if (candidate) {
    ...
    if (isTextElement(el.tagName)) {
        const candidate = textElementToCandidate(el, defaultFontFamily, defaultFontSize, viewBox, svgRoot);
        if (candidate) {
            candidates.push(candidate);
            ctx.textNodeCount += 1;
        }
...
export function extractTextDeckNodes(svgRoot, viewBox, defaultFontFamily, defaultFontSize, ctx) {
    const candidates = [];
...
export function extractTextBlocks(svgRoot, defaultFontFamily, defaultFontSize, ctx) {
    const viewBox = {
        minX: 0,
        minY: 0,
        width: 800,
