...
function extractGradientFunction(value) {
...
function parseGradientCoord(value, fallback) {
    if (!value) {
        return fallback;
    }
...
function normalizeStopOffset(offset, index, total) {
    if (!offset) {
        if (total <= 1) {
            return '0%';
        }
...
function getStopColor(stop) {
    const styleColor = stop.style?.getPropertyValue('stop-color');
    return stop.getAttribute('stop-color') ?? styleColor ?? '#000000';
}
function linearGradientDirection(x1, y1, x2, y2) {
...
function svgLinearGradientToCss(el) {
...
function svgRadialGradientToCss(el) {
    const stops = Array.from(el.querySelectorAll('stop'));
    if (stops.length === 0) {
        return null;
    }
...
function resolveSvgGradientUrl(fill, svgRoot) {
    const match = fill.match(/^url\(\s*#([^)]+)\s*\)$/i);
    if (!match) {
        return null;
    }
    const gradientEl = svgRoot.querySelector(`#${CSS.escape(match[1])}`) ??
...
function parseCssGradientText(style) {
    const clip = parseStyleValue(style, '-webkit-background-clip') ?? parseStyleValue(style, 'background-clip');
    if (clip?.toLowerCase() !== 'text') {
        return undefined;
    }
    const background = parseStyleValue(style, 'background-image') ?? parseStyleValue(style, 'background');
    if (!background) {
        return undefined;
    }
    return extractGradientFunction(background) ?? undefined;
}
function parseSolidColorFromStyle(style) {
...
export function parseTextColorFromStyle(style) {
...
export function parseTextColorFromFill(fill, svgRoot) {
    if (!fill) {
        return {};
    }
...
export function parseTextColor(options) {
    if (options.style) {
